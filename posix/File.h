...
public:
	...
	map mmap(void *addr, size_t length, int prot = PROT_READ | PROT_WRITE,
			int flags = MAP_PRIVATE, off_t offset = 0)
	{
	 ...
	 * Call splice
	 */
	ssize_t spliceTo(int fd_out, size_t len, loff_t *off_in = nullptr,
			loff_t *off_out = nullptr, unsigned int flags = 0)
	{
		ssize_t s;
