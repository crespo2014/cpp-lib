 ...
 */
class tcpSock {
    int sock_;                        ///< setting as mutable will allow invalidate this handle on copy constructor and assignment
    ...
    void bind(uint16_t port, uint32_t address = INADDR_ANY) {
        ...
        if (::bind(sock_, (struct sockaddr*) &sock_addr_, sizeof(sock_addr_)) != 0) {
            LOG(ERROR_INVALID_RESULT) << "failed to bind to socket. ip " << sock_addr_.sin_addr.s_addr << ":" << sock_addr_.sin_port;
            throw SockException("bind", &sock_addr_);
        }
    }
    void listen(int count = 5) {
        if (::listen(sock_, count) != 0) {
            ...
            throw SockException("listen", &sock_addr_);
        }
    }
    /// Constructor for single socket
    tcpSock(int type = SOCK_STREAM, int domain = AF_INET, int protocol = 0) {
        sock_ = socket(domain, type, protocol);
        if (sock_ < 0) {
            LOG(ERROR_INVALID_RESULT) << "failed to create socket. " << domain << " " << type << " " << protocol;
            throw OSException("socket");
        }
    ...
    void connect(const struct in_addr& address, uint16_t port) {
        ...
        if (::connect(sock_, (const sockaddr*) &sock_addr_, sizeof(sock_addr_)) != 0) {
            ...
            throw SockException("connect", &sock_addr_);
        }
    }
    /**
...
//    }
    ...
    void close() {
        if (::close(sock_) != 0) {
            LOG(ERROR_INVALID_RESULT) << "failed to close socket. ip " << sock_addr_.sin_addr.s_addr << ":" << sock_addr_.sin_port;
            throw OSException("close");
        }
    }
    /**
    ...
    void setBlockingMode(bool status) {
        ...
        if (::ioctl(sock_, FIONBIO, &opt) < 0) {
            ...
            throw SockException("ioctl", &sock_addr_);
        }
    }
    /**
    ...
    tcpSock accept() {
        ...
        if (fd < 0) {
            ...
            throw SockException("accept", &sock_addr_);
        }
        return {fd, sock_addr};
    }
    /**
...
//    }
     ...
     */
    static tcpSock getNonBlockingServer(int backlog, uint16_t port, uint32_t address = INADDR_ANY, int type = SOCK_STREAM, int family = AF_INET,
                                        int protocol = 0) {
    ...
    void setReadWriteTimeout_ms(int rd_ms, int wr_ms) {
        ...
        if ((setsockopt(sock_, SOL_SOCKET, SO_RCVTIMEO, &rd, sizeof(rd)) != 0)
                || (setsockopt(sock_, SOL_SOCKET, SO_SNDTIMEO, &wr, sizeof(wr)) != 0)) {
            throw OSException("setsockopt");
        }
    }
    /**
...
//            throw Faz::os::OSException("setsockopt");
//    }
    /**
