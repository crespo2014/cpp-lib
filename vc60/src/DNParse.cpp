...
char* CDNParse::GetKey(char *key, char *defval)
{
	...
	LOG_TERROR ERR_FATAL,"key '%s' not found",key);
	return defval;
}
...
void CDNParse::LogData()
{
	...
	for (i=0;i<m_key_count;i++)
	{
		LOG_TDATA "%s = %s",m_key[i].key,m_key[i].val);
	}
}
...
DWORD CDNParse::Parse_String(char *str)
{
	...
	/*
		...
		no puede haber espacios entre el separador y el valor porque seran considerados parte del valor
	...
	*/
	if (*str == 0) return LOG_TERROR ERR_FATAL,"%s Empty string not allowed ",fnc_name);
	m_data = (char*)m_page.CreatePage(strlen(str)+1); if (m_data == NULL) return LOG_TSTACK;
	...
	while (*cptr != 0)
	{
		...
		if (cptr == NULL)
		{
			return LOG_TERROR ERR_FATAL,"%s->Sign '=' is missing in ket No. %d",fnc_name,m_key_count);
		}
		...
		if (cptr == NULL) 
		{
			if (cend != cbreak) return LOG_TERROR ERR_FATAL,"%s terminator char '%c' not found",fnc_name,cend);
			cptr = strchr(cptr1,0);
		}
...
DWORD CDNParse::Key_Get(char *key, char *val, WORD maxsize)
{
	...
	return LOG_TERROR ERR_FATAL,"%s key '%s' not found",fnc_name,key);
}
...
DWORD CDNParse::Data_GetBuffer(char **buff, WORD len,char* fnc_name)
{
	...
	if (remained < len) return LOG_TERROR ERR_FATAL,"%s failed to get %d bytes from buffer, %d bytes remined",fnc_name,len,remained);
	*buff = m_data_ptr;
	m_data_ptr += len;
	return ERR_OK;
}
...
DWORD CDNParse::Data_AddStr(char *str, char **pos, char *fnc_name, DWORD len)
{
	...
	{
		*pos = NULL;
		return LOG_TERROR ERR_FATAL,"%s Invalid params in Data_AddStr",fnc_name);
	}

	DWORD remained = m_data_size - (m_data_ptr - m_data);
	if (len == 0) len = strlen(str);
	if (remained < len+1) return LOG_TERROR ERR_FATAL,"%s failed to get %d bytes from buffer, %d bytes remined",fnc_name,len,remained);
	*pos = m_data_ptr;
...
DWORD CDNParse::Key_ByIndex(WORD index, _DNKEY **pdnkey)
{
	...
	if (index >= m_key_count) return LOG_TERROR ERR_FATAL,"%s index out of bound",fnc_name);
	*pdnkey = &m_key[index];
	return ERR_OK;
}
