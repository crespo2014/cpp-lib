...
{
	char	*fnc_name = "CbLDAP::SetError_Add_ldap_code_msg";
	char	m_err_msg[300];
	char	*cptr;
	va_list arg_ptr;
	...
	*/
	return LOG_TERROR ERR_API,"%s",m_err_msg);
	return ERR_API;
}
...
{
	char	*fnc_name = "CbLDAP::Attribute_Next";
	ULONG			ulr;
	BerElement*		attr_ber1;
	if (m_ldap_entry == NULL) return LOG_TERROR ERR_EOF,"Invalid entry call Entry_Next First");
	if (m_attr_ber == NULL)
	{
		...
		if (m_attr_name == NULL)  
		{
			if (ulr != LDAP_SUCCESS) return SetError_Add_ldap_code_msg(ulr,"CbLDAP::Attribute_Next ... ");
				...
				return LOG_TERROR ERR_EOF,"CbLDAP::Attribute_Next ... EOF");
		}
...
{
	char	*fnc_name = "CbLDAP::Attribute_GetName";

	*str = 0;
	if (m_attr_name == NULL) return LOG_TERROR ERR_EOF,"CbLDAP::Attribute_GetName ... NULL");
...
DWORD CbLDAP::shearch_s(PCHAR base, ULONG scope, PCHAR filter, PCHAR attrs[], ULONG attrsonly)
{
	ULONG	ulr;
	ulr = ldap_search_s(m_hldap,base,scope,filter,attrs,attrsonly,&m_ldapmessage);
	if (ulr != LDAP_SUCCESS) return SetError_Add_ldap_code_msg(ulr,"CbLDAP::Search_s (%s,%s) ... ",base,filter);
	return ERR_OK;
}
