...
DWORD	CcmdLineParser::End()
{
	...
	if (mStatus == ST_NO_COUTE)
	{
		...
		//dwr = Push(' '); if (dwr != ERR_OK) return log_exit((dwr);
		dwr = endValue(); if (dwr != ERR_OK) return log_exit(dwr);
	}
...
DWORD	CcmdLineParser::Push(BYTE b)
{
	...
	case ST_NONE:
		{
			...
			if (mpMem->lock()	!= ERR_OK) return log_exit(ERR_NOMEMORY);		
	...
	if (mpMem->lockAdd(&b,1) != ERR_OK) return log_exit(ERR_NOMEMORY);	// se almacena el espacio porque es parte del valor
	return ERR_OK;
}
...
DWORD CcmdLineParser::endValue()
{
	...
	DWORD	size;
	BOOL	release = true;
	dwr = mpMem->lockAdd("",1);						if (dwr != ERR_OK) return log_exit(dwr);
	dwr = mpMem->lockCompact((void**)&cstr,&size);	if (dwr != ERR_OK) return log_exit(dwr);
	if (size != 1) 
	{
		dwr = mListener->OnValue_c(cstr,&release);	if (dwr != ERR_OK) return log_exit(dwr);
	}
	if (release)
	{
		dwr = mpMem->lockFree();	if (dwr != ERR_OK) return log_exit(dwr);
	}
	else
	{
		dwr = mpMem->lockClose();	if (dwr != ERR_OK) return log_exit(dwr);
	}
	mStatus = ST_NONE;
	return ERR_OK;
