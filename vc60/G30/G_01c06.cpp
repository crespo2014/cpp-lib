...
DWORD C3DViewGame::Initialize()
{
	...
	m_thread=::CreateThread(NULL,0,ThreadProc,this,THREAD_PRIORITY_NORMAL,&m_threadId);
	return 0;
}
...
DWORD C3DViewGame::GameStart()
{
	...
	if (CreateWindEx(/*WS_EX_TOPMOST*/0,NULL,WS_POPUP|WS_VISIBLE,0,0,(int)WWIDTH,(int)WHEIGHT,NULL,NULL,NULL)!=0)
		return ERR_NOWND;
	SetCapture(m_hWnd);
	SetFocus(m_hWnd);
...
DWORD C3DViewGame::ThreadProc(LPVOID pParam)
{
...
DWORD C3DViewGame::GameEnd()
{
	...
	m_message_time=INFINITE;
	m_play=false;
	PostMessage(m_hWnd,UM_GAME_END,0,0);
	return 0;
}
