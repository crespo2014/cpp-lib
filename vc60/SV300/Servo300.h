...
public:
	...
	DWORD ProcessDC(BYTE* Buffer,DWORD cant);
	DWORD DefineCurve();
	...
	*/
	...
	virtual DWORD _stdcall InitializeDevice();
private:
	float Convert_UI(int Value, int Channel);
	...
	DWORD ProcessDB(BYTE* Buffer,DWORD cant);
private:
	BYTE	m_CmdState;		// Maquina de estados del comando actual 
	/*
		RC - Read Curve
	*/
	WORD	m_RCtmpval;		// Temporal
	...
	*/
	...
	BYTE m_PP_Buff[4];	// utilizado para guardar 4 ascci y luego convertirlos a un numero
	SV_PP  Buff_PPunt[11];
	...
	DWORD AlarmStatus;
	DWORD TrendStatus;
	DWORD DefineBreathStatus;
	DWORD DefineCurveStatus;
	BYTE  m_CurrentCommand;	    // Comando que se esta procesando

	float Buff_Scale_Factor[30];
