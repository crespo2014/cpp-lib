// g_01c06.cpp : implementation file
//
...
BEGIN_MESSAGE_MAP(C3DViewGame, CWnd)
	//{{AFX_MSG_MAP(C3DViewGame)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()


/////////////////////////////////////////////////////////////////////////////
// C3DViewGame message handlers
...
DWORD C3DViewGame::Initialize()
{
	...
	m_thread=AfxBeginThread(ThreadProc,this,THREAD_PRIORITY_NORMAL,0,0,NULL);
	return 0;
}
...
/*
	Esta funcion da comienzo al juego
*/

DWORD C3DViewGame::GameStart()
{
	if (m_hWnd)
	{
		SetCapture();
	...
	if (!Create(NULL,NULL,WS_CHILD|WS_VISIBLE,rect,&Dlg,0,NULL))
		return ERR_NOWND;
		*/
	int MaxX=::GetSystemMetrics(SM_CXFULLSCREEN);
	int MaxY=::GetSystemMetrics(SM_CYFULLSCREEN);
	int x1=(MaxX-WWIDTH)/2;
	int y1=(MaxY-WHEIGHT)/2;
	if (!CreateEx(/*WS_EX_TOPMOST*/0,"STATIC",NULL,WS_POPUP|WS_VISIBLE,x1,y1,(int)WWIDTH,(int)WHEIGHT,NULL,NULL,NULL))
		return ERR_NOWND;
	
	SetCapture();
	SetFocus();
	/*
	m_hcursor=::LoadCursor(AfxGetInstanceHandle(),"colimador");
	SetCursor(m_hcursor);
	*/
	SetCursor(NULL);
	...
	case UM_GAME_END:
		{
			...
			if (m_end_time)
			{
				m_app->paciente.AlmacenarDatos(m_app->pDatos);			
			}
			else 
				m_app->paciente.AbortarCondicion();
			break;
		}
	case WM_KEYDOWN:
		{
...
UINT C3DViewGame::ThreadProc(LPVOID pParam)
{
	return ((C3DViewGame*)pParam)->ThreadMain();
}

UINT C3DViewGame::ThreadMain()
{
	while (true)
	{
		if (m_message.WaitMessage(m_message_time))
...
DWORD C3DViewGame::ThreadProcessMessage()
{
	int m_Key;
	S_MESSAGE	Msg;
	if (!m_message.PeekMessage(&Msg))
		return 0;
	switch (Msg.Msg)
	{
	case UM_INIT_OPENGL:
...
DWORD C3DViewGame::SetApp(CApp *pApp)
{
	m_app=pApp;
	return 0;
}
