...
{
public:
	DWORD WaitEvents(WORD* pwIOEvent,DWORD milliseconds);
	DWORD MsgWaitEvents(WORD* pwIOEvent,DWORD dwWakeMask = (QS_ALLEVENTS|QS_ALLINPUT,QS_ALLPOSTMESSAGE),DWORD milliseconds = INFINITE);
	DWORD OnWriteEvent();
	DWORD OnReadEvent(DWORD *pBytesRead);
	DWORD Read(BYTE* buffer,DWORD dwSize,DWORD* pBytesRead);
	DWORD Close();
