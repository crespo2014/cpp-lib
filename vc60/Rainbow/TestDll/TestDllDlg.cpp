...
{
	WORD	wr;
	wr = RNBOsproFormatPacket(hRnbKey,sizeof(hRnbKey));
}

void CTestDllDlg::OnButton2() 
{
	WORD	wr;
	wr = RNBOsproInitialize(hRnbKey);
}

void CTestDllDlg::OnButton3() 
{
	_RNBOsproInitialize* proc;
	WORD wr ;
	HMODULE hModule = ::LoadLibrary("H:\\users\\Lester\\Visual C++ 6.0\\Rainbow SentinelSuperPro\\sx32w\\sx32w.dll");	
	proc = (_RNBOsproInitialize*)::GetProcAddress(hModule,"RNBOsproInitialize");
	proc = (_RNBOsproInitialize*)::GetProcAddress(hModule,"_RNBOsproInitialize@4");
	wr=proc(hRnbKey);
}

void CTestDllDlg::OnButton4() 
{
	RNBOsproFindFirstUnit(NULL,0x56af);	
}
